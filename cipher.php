...
<div class="page-content">
	<div class="row">
	    ...
	    <div class="card">
	      <div class="card-body">
					<h6 class="card-title">Caesar Cipher Encode/Decode</h6>
					<form method="post" action="" class="needs-validation" novalidate>
						...
						<div class="form-group">
							...
							<input type="number" class="form-control" name="key" value="3" required="">
							<div class="valid-feedback">Good!!</div>
							<div class="invalid-feedback">Key can't be empty!!</div>
						</div>
		...
		<?php
			if (isset($_POST['resources']) && isset($_POST['key']) && !empty($_POST['resources']) && !empty($_POST['key'])) : 
				$input = htmlspecialchars($_POST['resources']);
				$key = htmlspecialchars($_POST['key']);
				if (isset($_POST['encode'])) {
					$result = Encipher($input, $key);
				} else {
					$result = Decipher($input, $key);
				}
		?>
		<div class="col-md-6 grid-margin stretch-card">
			<div class="card">
