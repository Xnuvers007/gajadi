...
<div class="page-content">
	<div class="row">
	    ...
	    <div class="card">
	      <div class="card-body">
					<h6 class="card-title">Bib Cipher Encode/Decode</h6>
		...
		<?php 
			$base = new Authcode();
			if (isset($_POST['resources']) && !empty($_POST['resources']) && isset($_POST['key']) && !empty($_POST['key'])) : 
				$input = htmlspecialchars($_POST['resources']);
				$key = htmlspecialchars($_POST['key']);
				if (isset($_POST['encode'])) {
					$result = $base->encode($input, $key);
				} else {
					$result = $base->decode($input, $key);
				}
		?>
		<div class="col-md-6 grid-margin stretch-card">
			<div class="card">
